...
bool OrcVisitor::visit(const std::shared_ptr<Werewolf>& defender) const {
    return false;
}

bool OrcVisitor::visit(const std::shared_ptr<Outlaw>& defender) const {
    return true;
}
...
Orc::Orc() : NPC(OrcType, 0, 0) {
    killDistance = 10, moveDistance = 20;
}
