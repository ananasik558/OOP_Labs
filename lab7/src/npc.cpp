...
NPC::NPC(CreatureType type, const std::pair<int,int> &coords, const int &attackRange, const int &movementRange) : 
    _coords{coords}, _type{type}, alive{true}, Attack{attackRange}, Move{movementRange} {}

NPC::NPC(CreatureType type, const int &x, const int &y, const int &attackRange, const int &movementRange) : 
    _coords{x,y}, _type(type), alive(true), Attack{attackRange}, Move{movementRange} {}
...
void NPC::move(const int& moveX, const int& moveY) {
    std::lock_guard<std::mutex> lck(mtx);
    if ((_coords.first + moveX >= 0) && (_coords.first + moveX <= SIZE)) {
        _coords.first += moveX;
    }

    if ((_coords.second + moveY >= 0) && (_coords.second + moveY <= SIZE)) {
